...
const main = async () => {
  ...
  */
  ...
  const osd = (await helper.getDeployedContract("MyERC20", await helper.getDeployedAddress('osd'))) as MyERC20;
  const _tokenIn = '0x0459b028fF1144751F21aab6042e821070536fed'
  ...
  const _isLong = true
  const _amountIn = '8796266733083087000'
  const _minOut = 0
  const _notionalDelta = '8787479253829256170'
  const _collateralPrice = 0
  const _indexPrice = 0
  await osd.approve(futureRouter.address, '87962667330830870000')

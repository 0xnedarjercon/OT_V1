...
const verifyContract = async (artifact: ZkSyncArtifact, contract: zk.Contract, constructorArgs: any[]) => {
  try {
    ...
    const line = `${new Date().toISOString()} ${contractName} ${contract.address} https://zksync2-testnet-explorer.zksync.dev/contract_verification/${await resp.text()}\n`

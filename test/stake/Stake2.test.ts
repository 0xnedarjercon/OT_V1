...
describe("Stake", async function () {
  ...
  describe("Stake unit", function () {
    it("Should end time ok ", async function () {
      ...
      await rewardToken.transfer(stake.address, '100000000000000000000000000000000')
    ...
    it("addRevenueToken", async function () {
      ...
      await stake.addRevenueToken(lpToken.address, btc.address);
      await stake.addRevenueToken(lpToken.address, ot.address);
      await expect(await stake.revenueInfoList(lpToken.address, 0)).to.be.equal(btc.address);
      await expect(await stake.revenueInfoList(lpToken.address, 1)).to.be.equal(ot.address);
      await expect(stake.addRevenue(lpToken.address, [btc.address], [])).to.be.revertedWith('tokenList eq amountList')
      await expect(stake.addRevenue(lpToken.address, [ot.address], [0])).to.be.revertedWith('addRevenue gt 0')
      await expect(stake.addRevenue(lpToken.address, [btcLp.address], [1])).to.be.revertedWith('revenueInfo not exists')
      await expect(stake.addRevenue(lpToken.address, [ot.address], [1])).to.be.revertedWith('no stake token')
      ...
      await minBlock(startTime, 1000)
      // await setNextBlockTimeAndMine(startTime, 1000);
      await btc.approve(stake.address, 10000000)
      await ot.approve(stake.address, 20000000)
      await stake.addRevenue(btcLp.address, [
        btc.address,
        ot.address
      ], [
        10000000,
      ...
      await stake.connect(alice).withdrawAccountRevenue(btcLp.address)
      await expect(await btc.balanceOf(alice.address)).to.be.equal(BigNumber.from('3333333'));
      await expect(await ot.balanceOf(alice.address)).to.be.equal(BigNumber.from('6666666'));
    })
  });
});

...
async function getTradeStakeUpdaterFactory() {
  return await ethers.getContractFactory("TradeStakeUpdater");
}
async function getTradeStakeFactory() {
...
type TradeStakeUpdaterFactory = Resolve<ReturnType<typeof getTradeStakeUpdaterFactory>>;
type TradeStakeUpdater = Resolve<ReturnType<TradeStakeUpdaterFactory["deploy"]>>;
type TradeStakeFactory = Resolve<ReturnType<typeof getTradeStakeFactory>>;
type TradeStake = Resolve<ReturnType<TradeStakeFactory["deploy"]>>;
...
describe("Swap", async function () {
  ...
  let TradeStakeUpdater: TradeStakeUpdaterFactory;
  let TradeStake: TradeStakeFactory;
  ...
  before(async function () {
    ...
    [Swap, ERC20, Osd, Borrow, TradeStakeUpdater, TradeStake] = await Promise.all([
      getSwapFactory(),
      getERC20Factory(),
      getOsdFactory(),
      getBorrowFactory(),
      getTradeStakeUpdaterFactory(),
      getTradeStakeFactory()
    ]);
  });
  ...
  beforeEach(async function () {
    await Promise.all([
      ...
      (async () => (ot = await ERC20.deploy("DAI", "DAI", 18)))(),
      (async () => {
    ...
    tradeStake = await TradeStake.deploy()
    tradeStakeUpdater = await TradeStakeUpdater.deploy(swap.address, osd.address, mockOracle.address, mockOracle.address, tradeStake.address);
    // setup tradeStake
  ...
  describe("removeLiquidity()", async function () {
    ...
    it("Should mint osd after swap", async function () {
      ...
      await tradeStakeUpdater.swapIn(btc.address,
        eth.address,
        e8(1),
        e18(21),
        owner.address,
  ...
  it("Should rebalance", async function () {
    {
      ...
      await btc.approve(swap.address, e8(1));
      await tradeStakeUpdater.swapIn(
        btc.address,
        eth.address,
        e8(1),
        e18(19),
      ...
      expect(await lToken.balanceOf(owner.address)).to.equal("10100151278");

      await btc.approve(swap.address, e8(3000));
      await tradeStakeUpdater.swapIn(btc.address, eth.address, e8(3000), 0, alice.address, 0);
      ...
      expect(await lToken.balanceOf(owner.address)).to.equal("320196883568");
    }

    {
      ...
      expect(await lToken.balanceOf(owner.address)).to.equal(e(100, 6));

      await usdc.approve(swap.address, e6(1));
      await tradeStakeUpdater.swapIn(usdc.address, eth.address, e6(1), 0, alice.address, 0);
      ...
      expect(await lToken.balanceOf(owner.address)).to.equal("101000300");

      await usdc.approve(swap.address, e6(3000));
      await tradeStakeUpdater.swapIn(usdc.address, eth.address, e6(3000), 0, alice.address, 0);
      ...
      expect(await lToken.balanceOf(owner.address)).to.equal("3102167869");
    }
  });
  ...
  it("Should not rebalance", async function () {
    ...
    await tradeStakeUpdater.swapIn(luna.address, eth.address, e18(210), e18(1), alice.address, nextBlockTimestamp)
    await expect(
      swap
        .connect(alice)
  ...
  describe("ISwapForBorrow", function () {
    it("Should call borrow on reserve change", async function () {
      ...
      await tradeStakeUpdater.swapOut(
        eth.address,
        btc.address,
        e18(99),
        e8(4),
      ...
      console.log("tradeStake pending ", await tradeStake.pending(alice.address))
